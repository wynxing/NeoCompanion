# NeoCompanion 总景产品需求文档

## Document Header

- **Title**: NeoCompanion Product Requirements Document (Overview)
- **Owner**: Product
- **Status**: Draft (Aligned with v3.3 Knowledge Workspace Decision)
- **Version**: 3.3
- **Last Updated**: 2026-06-18
- **Audience**: Product, Design, Engineering
- **Related Docs**:
  - [`README.md`](../README.md)
  - [`docs/具体能力构思.md`](./具体能力构思.md)
  - [`docs/WALLPAPER_STATUS_LAYER.md`](./WALLPAPER_STATUS_LAYER.md)

---

## 1. Document Purpose

本文档定义 NeoCompanion 的长期产品方向、核心产品判断、能力边界和阶段路线。

本文档是 NeoCompanion 的产品主文档，用于为后续版本规划、设计决策、技术讨论和架构设计提供统一依据。

---

## 2. Problem Statement

### 2.1 Market Gap

当前 AI 产品主要集中在三类能力：
1. 问答与生成
2. 效率与任务处理
3. 陪伴与角色互动

这些产品存在以下局限性：
- **纯问答型 AI**：缺乏持续陪伴状态，用户需要反复手动复制上下文，难以融入脑力活动的真实过程。
- **效率管理工具**：冰冷机械，缺乏温度，容易沦为“任务记录本”，甚至带给用户被监控的心理压力。
- **纯陪伴类产品**：偏重情感和娱乐聊天，难以参与实质性的学习、编程、创作等复杂脑力行为。
- **自主型 Agent**：过度追求全自动代替用户做决策，面临极高的出错率和隐私侵入性。

### 2.2 User Problem

目标用户（如学生、开发者、创作者等脑力活动者）的痛点在于：
- **难以开始**：面对复杂学习或工作任务时容易拖延，缺乏仪式感和即时正面反馈。
- **难以持续**：专注时容易被打断、分心或在多窗口切换中迷失。
- **卡壳与焦虑**：遇到高难度问题思路中断时，没有懂你的伙伴给予引导或舒缓情绪。
- **连接性欠缺**：自己写的辅助脚本、编译结果、监控状态等，无法以自然、低门槛的方式转化为日常感知。

### 2.3 Why This Matters

NeoCompanion 致力于通过**「助手情感载体 + 屏幕状态感知 + 本地知识工作空间 + 开放 Hook API」**的融合架构，解决这一痛点，让 AI 从“一次性应答工具”升级为“懂状态、能引用个人知识、长久相伴的数字搭子”。

---

## 3. Product Definition

### 3.1 Positioning

NeoCompanion 是一款**融合了「桌面悬浮智能助手 + 陪伴式监督学习 + 本地知识工作空间」的长期陪伴型 AI 系统**。

以可爱、有温度的悬浮助手（2D 精灵图帧动画）为前端载体，通过屏幕状态感知的温情反馈，辅以项目、笔记、任务与看板，将用户自己的资料转化为可检索、可引用的 AI 上下文，帮助用户在学习和创作中克服拖延、卡壳与中断。

NeoCompanion 的界面分为三层：
- **壁纸层**：沉浸式状态画布——天气、时间、任务进度、专注主控盘等信息融入桌面壁纸，支持一级轻交互（开始/暂停专注、点击任务跳转面板、沉浸模式切换）；
- **悬浮层**：交互入口——助手形象、语音气泡、Hook 角标通知，所有点击和右键操作在此完成；
- **面板层**：编辑操作——任务管理、AI 对话、设置配置、Hook 审批，按需弹出。

这种分层设计让状态信息像环境一样自然存在，不需要任何操作即可一瞥而知，而轻量操作可在壁纸层就近完成，复杂交互则集中在悬浮和面板层，避免功能边界模糊。

### 3.2 Core Product Decisions

本文档锁定以下产品核心判断：
1. **产品形态与主轴**：以桌面悬浮助手及拟人化 TTS 语音互动为核心界面，主打”贴心但不过分亲密的陪伴监督”，绝非机械式效率看板。
2. **状态感知温情化**：屏幕识别唯一目的是为了“感知用户当前的专注/分心/卡壳/疲劳状态”，并由助手给予匹配的情绪反馈（如递虚拟咖啡、催促休息），严禁做任何商业监控。
3. **本地知识工作空间与 AI 对话**：
   - 不从零做庞大笨重的 Autonomous Agent，也不把自动操作其它软件作为产品主轴。
   - 在单一本地工作空间中，以项目关联 Markdown 笔记、看板和统一任务。
   - AI 对话优先使用本地全文与向量混合检索，回答必须展示可点击来源；索引不可用时明确降级为全文检索。
4. **极客定制与 Hook 连接**：核心功能包含开放的 Hook API 监听接口，支持用户用自己编写的自定义程序推送编译、监控等状态提醒，助手进行拟人化播报。
5. **壁纸层状态画布 + 轻交互**：壁纸层以状态展示为主，支持一级交互（导航/触发型点击，如开始/暂停专注、点击任务跳转面板、沉浸模式切换），不承载二级及以上交互（编辑、输入、复杂选择）。二级交互（勾选任务、调整时长、处理 Hook 通知等）统一由悬浮层和面板层完成。这确保了"信息在哪里、操作就离哪里最近"的就近交互原则，同时保持壁纸层的安静氛围。

---

## 4. Target Users

### 4.1 Core User

- **学生群体**：用于长期学业规划、番茄钟陪同专注、背词与论文写作卡壳引导，对正面情感反馈敏感。
- **开发者与极客群体**：用于长期的深夜写代码陪伴、代码错误通过 Claude 调试面板适配、构建与服务器监控 Hook 拟人化提醒。
- **创作者群体**：用于灵感随手记、日常琐碎待办打理、写作进度监督。

---

## 5. Capability Model

NeoCompanion 的长期能力分为四层：

```
+-------------------------------------------------------+
|  1. Assistant Layer (助手悬浮交互 / TTS 语音 / 情绪反馈 /  |
|                         壁纸层纯状态显示)                |
+-------------------------------------------------------+
|  2. Routine Layer   (番茄钟 / Todo账本 / 天气 / 助手小日记) |
+-------------------------------------------------------+
|  3. Knowledge & AI  (项目 / 笔记 / 看板 / 混合检索 / AI 对话) |
+-------------------------------------------------------+
|  4. Hook & System   (本地安全Hook API / 本地状态感知与记忆) |
+-------------------------------------------------------+
```

### 5.1 Assistant Layer (助手交互层)
- **2D 精灵图帧动画**：桌面常驻，支持拖拽、置顶、多种助手动作响应。形象不一定是人物——可以是动物、机器人、抽象图形。
- **TTS 语音合成**：拟人化声线和语气交互，告别冷冰冰的纯文字提示。
- **状态化情绪反馈**：配合用户状态触发对应动作，语气温和专业，贴心但不过分亲密。
- **壁纸层状态画布**：将天气时间、任务清单、专注主控盘、助手寄语、氛围色调等信息融入桌面壁纸背景层。以展示为主，支持一级轻交互——用户回到桌面即可一瞥状态，需要操作时可就近点击，复杂操作仍通过悬浮助手或面板完成。
- **Hook 角标通知**：当外部 Agent 推送通知或审批请求时，悬浮助手右上角显示红点/数字角标（类似手机 App 未读提示），用户点击角标后跳转面板处理。

### 5.2 Routine Layer (日常琐碎陪伴层)
- **陪伴式番茄钟**：助手与用户同步进入专注计时，陪伴小动作。
- **助手任务清单**：用账本包装 Todo，拟人化语气催促和打气。
- **日常天气与碎碎念**：基于气象的保暖带伞提醒、早晚问候、健康作息劝导。
- **助手工作日志**：日终自动生成以助手口吻撰写的温度成长记录。

### 5.3 Knowledge & AI Layer (本地知识与 AI 层)
- **单一本地工作空间**：项目是顶层组织单元；每个项目关联 Markdown 笔记、看板与任务，未分类内容进入默认“收件箱”。项目支持嵌套：非底层项目以卡片网格展示其子项目，底层项目展开为笔记/看板/任务工作区。
- **统一任务模型**：现有简单待办升级为任务清单视图；同一任务可归属项目、看板与列，避免维护两套任务数据。看板列按项目自定义，任务支持跨列拖拽、列内排序和完成状态。
- **双向链接**：Markdown 笔记正文支持 `[[目标]]` 或 `[[目标|显示文本]]` 语法，点击可跳转到对应笔记或任务，并在目标详情展示 backlinks。
- **混合知识检索**：SQLite FTS5 负责标题、正文、标签和专有名词检索，`sqlite-vec` 负责语义召回，两路结果融合排序。
- **有来源的 AI 回答**：AI 默认检索当前项目，也允许检索全部项目；回答后展示实际召回的笔记或任务来源。
- **可用性优先**：Embedding 未配置、额度不足或索引失败时，笔记、看板和全文检索继续工作，AI 知识问答使用关键词召回并明确显示降级状态。

### 5.4 Hook & System Layer (系统与 Hook 层)
- **安全本地多通道 Hook**：通过 HTTP、WebSocket、File Watcher 或本地域套接字接收用户自定义脚本/程序推送；不主动扫描或修改第三方工具配置。
- **浮动权限审查气泡**：支持当外部 Agent 执行敏感的写操作或命令时，通过助手弹出半透明审批气泡 (Permission Bubble)，允许用户通过快捷热键一键审批，实现终端与桌面的温情交互。
- **隐私感知引擎**：本地运行窗口特征提取，去除敏感及隐私正文，仅用最小必要维度去推算脑力状态。
- **本地长期记忆**：记录用户专注偏好、被提醒习惯，使助手在陪伴过程中更加默契。

---

## 6. Roadmap Framework

> **Scope note**: v1 是当前正在实现的最小闭环；v2 与 v3 是后续规划方向，尚未全部交付。具体实现状态请对照 `docs/ARCHITECTURE.md` 中的 `(Planned)` / `(Partial)` 标注。

### 6.1 v1: 助手最小闭环 (Warm Companion Base)
- **目标**：实现桌面常驻悬浮助手的基本 2D 精灵图形态、语音 TTS 播报。
- **核心功能**：助手番茄钟、助手待办、天气碎碎念。
- **感知反馈**：检测活跃窗口类型，对专注与严重分心进行温和提醒。
- **壁纸层 MVP**：实现壁纸窗口嵌入桌面图标层下方，展示天气时间与专注主控盘。验证 WorkerW 技术可行性与用户体验。
- **Hook 角标**：助手右上角红点/数字角标提示待处理通知。

### 6.2 v2: 本地知识工作空间与知识增强对话 (Local Knowledge Workspace & RAG)
- **目标**：完成“记录 → 组织 → 检索 → 引用回答”的本地知识闭环。
- **前置状态**：v3.3 已交付知识工作空间前端交互预览版（卡片化项目浏览器、嵌套项目、笔记/看板/任务工作区、双向链接、全局主题切换），当前为 mock 数据驱动。
- **核心功能**：
  - 项目、Markdown 笔记、看板和统一任务管理（后端持久化与检索接入）。
  - SQLite FTS5 + `sqlite-vec` 混合检索、增量索引、索引状态和手动重建。
  - 可配置的聊天模型与 OpenAI-compatible Embedding Provider；密钥只进入系统凭据存储。
  - AI 回答附可点击来源；向量能力失败时降级全文检索。
  - 通用 Hook API、文件监听与权限审批作为独立能力保留，不绑定特定外部工具。

### 6.3 v3: 本地知识扩展与长期陪伴 (Knowledge Expansion & Long-term Companion)
- **目标**：扩展知识来源，同时保持本地优先和用户可控。
- **核心功能**：
  - 指定 Markdown 文件夹同步，以及 PDF、网页等多格式导入。
  - 可选本地 embedding 模型，降低知识正文发送到云端的需要。
  - 将用户知识、对话记录与助手长期记忆分区治理，提供独立开关、来源和删除能力。
  - 在显式授权下演进助手偏好记忆，形成有依据、可撤销的个性化交互。
