...
- **壁纸层**：纯状态显示——天气、时间、任务进度、专注计时等信息融入桌面壁纸，只展示不交互；
- **悬浮层**：交互入口——助手形象、语音气泡、Hook 角标通知，所有点击和右键操作在此完成；
- **面板层**：编辑操作——任务管理、AI 对话、设置配置、Hook 审批，按需弹出。

这种分层设计让状态信息像环境一样自然存在，不需要任何操作即可一瞥而知，而交互行为则集中在悬浮和面板层完成，避免功能边界模糊。
...
5. **壁纸层纯状态显示**：壁纸层只负责状态信息的展示（天气、时间、任务、专注计时、助手寄语、氛围色调），不承载任何交互行为。所有交互（勾选任务、开始专注、处理 Hook 通知等）统一由悬浮层和面板层完成。这确保了"状态感知零操作、交互执行有入口"的清晰分工。
...
- **壁纸层状态显示**：将天气时间、任务清单、专注计时、助手寄语、氛围色调等信息融入桌面壁纸背景层。纯展示、零交互——用户回到桌面即可一瞥状态，所有操作仍通过悬浮助手或面板完成。
...
- **壁纸层 MVP**：实现壁纸窗口嵌入桌面图标层下方，展示天气时间与专注计时圆环。验证 WorkerW 技术可行性与用户体验。
