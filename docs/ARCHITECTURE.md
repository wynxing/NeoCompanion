...
Embedding provider 配置（provider/baseUrl/model/apiKey）持久化到 `app_config` 表，sidecar 重启不丢失。apiKey 双通道：PUT 传 `apiKey` 则明文落库（仅存本地 db 文件，不进 git，GET 不回显）；传 `apiKey:null` 则清空落库值、运行时回退环境变量 `EMBEDDING_API_KEY`。GET 端点返回 `apiKeySource: "stored" | "env" | "none"` 供前端提示。
