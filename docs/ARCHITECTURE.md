...
由于桌宠常驻桌面且背景完全透明，为避免透明空白画布遮挡用户操作背后的窗口，同时保证桌宠角色本身能被拖拽和点击，系统采用 **Canvas 像素级 Alpha 感知与动态事件拦截设计**：

1. **WebView 动态热区检测**：
   在 React WebView 顶层组件的 Canvas/Container 上绑定 `mousemove` 事件监听：
   ```typescript
   // apps/desktop/src/features/companion/components/PetContainer.tsx
   const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
     ...
     if (alpha === 0) {
       ...
       // 宠物角色实体区：使窗口拦截鼠标，支持点击与拖拽
       appWindow.setIgnoreCursorEvents(false);
     }
   };
   ```
2. **边缘保护与防抖**：
   窗口失焦后或离开宠物热区时，自动重置 `setIgnoreCursorEvents(false)`，保障拖拽操作的稳定连贯。
...
| POST | `/api/hook/push` | 外部用户自定义脚本/通知状态推送 | v1 |
...
* **离线降级状态**：若检测到网络中断或 API 失联，系统自动降级无缝回退至浏览器内置的 **Web Speech API (`window.speechSynthesis`)**。
  ...
  * 此策略**0 体积开销**（无需本地捆绑 100MB+ 的 Sherpa-ONNX 离线包），且**100% 离线可用**，能够确保在断网环境下桌宠绝不失声，实现长效陪伴。
...
3. WebView (React 前端) 弹窗半透明 Permission Bubble 卡片
   → 展示提示、敏感度和操作项
...
2. **高频事件内存队列防抖合并（Debounced Write Queue）**：
   ...
   * 这一合并机制可降低 SQLite 95% 以上的写事务吞吐，极大保护了用户的固态硬盘寿命，并消除桌宠动作渲染卡帧现象。

---

## 10. Hook & Permission Control System Architecture

Hook 机制与权限控制系统是 NeoCompanion 接入外部生态（如 OpenClaw）并实现安全保护与实时状态感知的基础。
...
1. **OpenClaw 配置扫描**：
   ...
   - 行为：若存在，解析 JSON 内容，检查 `plugins` 或 `hooks` 段中是否已注册 NeoCompanion 的 Hook 端点。
...
外部 Agent 的微观状态（Thinking, Typing, Success, Error）通过 Hook 主动推送给 Fastify，再流式同步到 WebView 用于 Live2D 拟人化动画状态机：
```typescript
...
```
[System Global Shortcut]                     [Tauri Rust Core]                      [React WebView]
           │                                         │                                      │
           │  Press Ctrl+Shift+Y                     │                                      │
           ├────────────────────────────────────────▶│                                      │
           │                                         │ ──┐                                  │
...
在 Rust Core 的入口中注册快捷键，拦截后向 React WebView 派发自定义 Tauri 事件：
...
pub fn register_shortcuts(app: AppHandle) {
    ...
    shortcut_manager.register("Ctrl+Shift+Y", move || {
        // 向当前处于激活/展示状态的桌面浮窗广播快捷键事件
        if let Some(window) = app_clone_y.get_window("main") {
            let _ = window.emit("shortcut:approve", ());
        }
    }).unwrap();
    ...
    shortcut_manager.register("Ctrl+Shift+N", move || {
        if let Some(window) = app_clone_n.get_window("main") {
            let _ = window.emit("shortcut:deny", ());
        }
    }).unwrap();
}
```

#### 10.4.2 React WebView 热键事件监听与 WS 反射
在 React 前端中，通过 `use-tauri.ts` 钩子订阅这两个全局快捷键事件。当事件被捕获后，WebView 更新自身的 Permission Bubble UI 渲染状态，并生成 WS 消息返回给 Fastify，完美复用现有的 WebSocket 解挂管线：

```typescript
// apps/desktop/src/features/companion/hooks/usePermissionShortcuts.ts
...
export function usePermissionShortcuts() {
  ...
  const { pendingRequests, removeRequest } = useCompanionStore();

  useEffect(() => {
    // 监听 Tauri 系统级放行快捷键
    const unlistenApprove = listen('shortcut:approve', () => {
      ...
      if (latestRequest) {
        ...
        removeRequest(latestRequest.requestId);
      }
    });

    // 监听 Tauri 系统级拒绝快捷键
    const unlistenDeny = listen('shortcut:deny', () => {
      const latestRequest = pendingRequests[pendingRequests.length - 1];
      if (latestRequest) {
        sendWsMessage({
          type: 'permission:response',
  ...
  }, [pendingRequests, sendWsMessage, removeRequest]);
}
```
...
### 12.1 v1: Single-Scenario Companion (最小闭环)
